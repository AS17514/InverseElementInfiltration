...
namespace TheLaw.UI
{
    ...
    public static class EventTexts
    {
        ...
        static readonly Dictionary<string, string> Descs = new Dictionary<string, string>
        {
            ...
            ["deck_standard"] = "你进入棋盘前最后的整备地点，你可以调整随身的棋子牌组，毕竟待会要面对的是真正的敌人。\n也许你也可以就此打道回府，“伊甸”的宽宏大量会原谅这一次的小小入侵。\n<i>真的不能操控这些机械臂去打架吗，感觉真的会很强诶。</i>",
            ["rule_pick"] = "你们来到了终端面前，这是一面黑墙，背后就是“牧场”庞大的后端。你可以为这个游戏添上你的玩法，给“伊甸”一份沉重的“大礼”。这也将会伴随游戏的整个过程。\n你可以一脉相承，也可以试试不同的组合。\n<i>兔子，坦克，最佳搭配！</i>",
        };

        /// <summary>事件 id 归一化：资产名带 "Event_" 前缀（如 Event_ability_pick）——查表键不带前缀（2026-08-26 测试反馈修复）。</summary>
        static string NormalizeKey(string eventId)
        {
            if (string.IsNullOrEmpty(eventId)) return eventId;
            const string prefix = "Event_";
            return eventId.StartsWith(prefix, System.StringComparison.Ordinal) ? eventId.Substring(prefix.Length) : eventId;
        }

        public static string TitleFor(string eventId, EventDefinition fallback)
        {
            var key = NormalizeKey(eventId);
            if (!string.IsNullOrEmpty(key) && Titles.TryGetValue(key, out var t)) return NormalizePlus(t);
        ...
        {
            var key = NormalizeKey(eventId);
            if (!string.IsNullOrEmpty(key) && Descs.TryGetValue(key, out var d))
            {
