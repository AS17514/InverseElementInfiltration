...
namespace TheLaw.UI
{
    ...
    /// 前端显示层覆盖——能力/编辑/构筑三事件标题与描述（TMP 富文本 <i> 斜体；{0} = 动态刷新次数）；
    /// 未登记事件回退事件定义原文本（events.json → EventDefinition 资产，数据域未动）。
    /// 玩法选择事件（rule_pick）暂未接入（2026-08-25 用户定案，先不考虑）。
    /// </summary>
    public static class EventTexts
    {
        ...
        {
            if (!string.IsNullOrEmpty(eventId) && Titles.TryGetValue(eventId, out var t)) return t;
            return fallback != null && !string.IsNullOrEmpty(fallback.title) ? fallback.title : "未知事件";
        }
        ...
        public static string DescFor(string eventId, EventDefinition fallback, int? refreshTotal = null)
        {
            ...
            if (fallback != null && !string.IsNullOrEmpty(fallback.description)) return fallback.description;
            return fallback != null && !string.IsNullOrEmpty(fallback.title) ? fallback.title : string.Empty;
        }
    }
}
